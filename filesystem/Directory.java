...
public class Directory {
	...
	/**
	 * The following class defines a "directory entry", representing a file in the
	 * directory. Each entry gives the name of the file, and where the file's header
	 * is to be found on disk.
	 * 
	 * Internal data structures kept public so that Directory operations can access
	 * them directly.
	 */
	...
	/**
	 * Initialize a directory; initially, the directory is completely empty. If the
	 * disk is being formatted, an empty directory is all we need, but otherwise, we
	 * need to call FetchFrom in order to initialize it from disk.
	 * 
	 * @param pSize
	...
	public void fetchFrom(OpenFile pFile) {
		...
		for (int i = 0; i < mTableSize; i++) {
			...
			/*
			 * String sName = new String(mTable[i].mName); byte bName = sName.getBytes();
			 * byte nameBuffer = new byte[FILENAMEMAXLEN * 2]; System.arraycopy(bName,0,
			 * nameBuffer,0, Math.min(bName.length,nameBuffer.length));
			 * System.arraycopy(nameBuffer, 0, buffer, i * sizeOfDirectoryEntry() +
			 * 5,nameBuffer.length );
			 */
		}
	 ...
	 * @return -1 if the name isn't in the directory, the location in the table of
	 *         directory entries where the file is saved.
	 */
	...
	/**
	 * Look up file name in directory, and return the disk sector number where the
	 * file's header is stored. Return -1 if the name isn't in the directory.
	 * 
	 * @param pName
	 *            the file name to look up
	 * @return the disk sector number where the file's header is stored. -1 if the
	...
	/**
	 * Add a file into the directory. Return TRUE if successful; return FALSE if the
	 * file name is already in the directory, or if the directory is completely
	 * full, and has no more space for additional file names.
	 * 
	 * @param PName
	...
	/**
	 * List all the file names in the directory, their FileHeader locations, and the
	 * contents of each file. For debugging.
	 */
	public void print() {
