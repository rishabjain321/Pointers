...
public class FileHeader {
	...
	/**
	 * Initialize a fresh file header for a newly created file. Allocate data blocks
	 * for the file out of the map of free disk blocks.
	 * 
	 * 
	 * @param pFreeMap
	...
	public void fetchFrom(int sector) {
		...
		for (int i = 0; i < mNumSectors && i < mDataSectors.length; i++) {
	...
	/**
	 * Converts virtual address to sector number. This is essentially a translation
	 * from a virtual address (the offset in the file) to a physical address (the
	 * sector where the data at the offset is stored).
	 * 
	 * @param offset
	 *            the location within the file of the byte in question
	 * @return The disk sector which is storing a particular byte within the file.
	 */
	public int byteToSector(int offset) {
		//System.out.println(offset + "is at: " + (mDataSectors[offset / Disk.SectorSize]));
		int vbn = [offset / Disk.SectorSize];
		//1.Check to see if this offset fails in the direct pointer block:
		//  [0,NumDirect]
		if(vbn < 28)
			return (mDataSectors[vbn]);
		//2.Check to see if this offset fall in the singly indirect pointer block:
		//  [NumDirect, SinglyDirect Size + NumDirect]
		if(vbn => 28 && vbn < 60)
			return (mDataSectors[vbn]);
		//3. Check to see if the offset fall in the doubly indirect range:
		//	[SinglyDirect Size + NumDirect,
		// SinglyDirect Size + NumDirect + ]
		if(vbn => 61 && vbn < 1024)
			return (mDataSectors[vbn]);
	...
	public void print() {
		...
		for (i = k = 0; i < mNumSectors; i++) {
			 ...
			 * System.out.println(new String(data[j])); else System.out.println(new
			 * String(data[j]));
			 */
			// }
		}
