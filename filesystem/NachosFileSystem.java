...
public class NachosFileSystem implements FileSystem {
	...
	/**
	 * Initialize the file system. If format = true, the disk has nothing on it, and
	 * we need to initialize the disk to contain an empty directory, and a bitmap of
	 * free sectors (with almost but not all of the sectors marked as free).
	 * 
	 * If format = false, we just have to open the files representing the bitmap and
	 * the directory.
	 * 
	 * @param pFormat
	...
	/**
	 * Create a file in the Nachos file system (similar to UNIX create). Since we
	 * can't increase the size of files dynamically, we have to give Create the
	 * initial size of the file.
	 * 
	 * The steps to create a file are: 1) Make sure the file doesn't already exist
	 * 2) Allocate a sector for the file header 3) Allocate space on disk for the
	 * data blocks for the file 4) Add the name to the directory 5) Store the new
	 * file header on disk 6) Flush the changes to the bitmap and the directory back
	 * to disk
	 * 
	 * Return true if everything goes ok, otherwise, return false.
	 * 
	 * Create fails if: 1) file is already in directory 2) no free space for file
	 * header 3) no free entry for file in directory 4) no free space for data
	 * blocks for the file
	 * 
	 * Note that this implementation assumes there is no concurrent access to the
	 * file system!
	 * 
	 * @param pName
	...
	/**
	 * Open a file for reading and writing. To open a file: Find the location of the
	 * file's header, using the directory Bring the header into memory
	 * 
	 * @param pName
	 ...
	 * contents of the directory for each file in the directory, the contents of the
	 * file header the data in the file
	 */
	public void print() {
