...
public class NachosOpenFile implements OpenFile {
	...
	/**
	 * Open a Nachos file for reading and writing. Bring the file header into memory
	...
	/**
	 * Change the current location within the open file -- the point at which the
	 * next Read or Write will start from.
	 * 
	 * @param position
	 ...
	 * number of bytes actually written or read, and as a side effect, increment the
	...
	/**
	 * Read/write a portion of a file, starting at "position". Return the number of
	 * bytes actually written or read, but has no side effects (except that Write
	 * modifies the file, of course).
	 * 
	 * There is no guarantee the request starts or ends on an even disk sector
	 * boundary; however the disk only knows how to read/write a whole disk sector
	 * at a time. Thus:
	 * 
	 * For ReadAt: We read in all of the full or partial sectors that are part of
	 * the request, but we only copy the part we are interested in. For WriteAt: We
	 * must first read in any sectors that will be partially written, so that we
	 * don't overwrite the unmodified portion. We then copy in the data that will be
	 * modified, and write back all the full or partial sectors that are part of the
	 * request.
	 * 
	 * @param into
	 *            the buffer to contain the data to be read from disk
	 ...
	 *            the number of bytes to transfer
	 * @param position
	 *            the offset within the file of the first byte to be read/written
	 *            --------------------------------------------------
	 *            --------------------
	 */
