...
public class AddrSpace {
	 ...
	 * "executable", and set everything up so that we can start executing user
	 * instructions.
	 * 
	 * Assumes that the object code file is in NOFF format.
	 * 
	 * First, set up the translation from program memory to physical memory. For
	 * now, this is really simple (1:1), since we are only uniprogramming, and we
	 * have a single unsegmented page table
	 * 
	 * @param executable
	...
	public AddrSpace(OpenFile executable) {
		...
		assert (mNumPages <= mFreeMap.numClear());
		...
		for (int i = 0; i < mNumPages; i++) {
			...
			/*
			 * write bytes to the swap space 1. create a disk map for this process 2. copy
			 * current location in the swap space into the first entry of the disk map
			 */
			diskmap[i] = JNachos.swapspace_counter;
			mPageTable[i].trans_diskmap = JNachos.swapspace_counter;
			/* copy the swapspace address to the pagetable entry */
	 ...
	 * immediately jump to user code. Note that these will be saved/restored into
	 * the currentThread.userRegisters when this thread is context switched out.
	 */
	...
	/**
	 * On a context switch, restore the machine state so that this address space can
	 * run.
	 * 
