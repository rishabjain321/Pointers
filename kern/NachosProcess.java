 ...
 * provisions.
 * 
 * Created by Patrick McSweeney on 12/5/08. Copyright 2008 Patrick J. McSweeney All rights reserved.
 */
...
public class NachosProcess implements Runnable {
	...
	public static Hashtable<Integer, NachosProcess> mProcessHash = new Hashtable<Integer, NachosProcess>();
	...
	public NachosProcess(String pProcessName) {
		...
		// assign a value to processid
		processid = counter++;

		// insert the pid and its corresponding object into the hashtable just
		// created
		mProcessHash.put(this.getid(), this);
	}
	 ...
	 * level threads. So if this is the first time the thread is running we have to
	 * start the java thread otherwise we resume it.
	 **/
	 ...
	 * passed to the procedure, it is possible to pass multiple arguments by making
	 * them fields of a class, and passing an object to the structure as "arg".
	 * 
	 * 
	 * @param pFunc
	 ...
	 * re-scheduled.
	 * 
	 * NOTE: Returns immediately if no other Process on the ready queue. Otherwise
	 * returns when the Process eventually works its way to the front of the ready
	 * list and gets re-scheduled.
	 * 
	 * NOTE: We disable interrupts, so that looking at the Process on the front of
	 * the ready list, and switching to it, can be done atomically. On return, we
	 * re-set the interrupt level to its original state, in case we are called with
	 * interrupts disabled.
	 * 
	 ...
	 * synchronization variable (Semaphore, Lock, or Condition). Eventually, some
	 * Process will wake this Process up, and put it back on the ready queue, so
	 * that it can be re-scheduled.
	 * 
	 * NOTE: If there are no Processs on the ready queue, that means we have no
	 * Process to run. "Interrupt::Idle" is called to signify that we should idle
	 * the CPU until the next I/O interrupt occurs (the only thing that could cause
	 * a Process to become ready to run).
	 * 
	 * NOTE: we assume interrupts are already disabled, because it is called from
	 * the synchronization routines which must disable interrupts for atomicity. We
	 * need interrupts off so that there can't be a time slice between pulling the
	 * first Process off the ready list, and switching to it.
	 **/
	public void sleep() {
		NachosProcess nextProcess;
	...
	/**
	 * Dispatch the CPU to nextProcess. Save the state of the old process, and load
	 * the state of the new process, by calling the machine dependent context switch
	 * routine, SWITCH.
	 * 
	 * Note: we assume the state of the previously running process has already been
	 * changed from running to blocked or ready (depending). Side effect: The global
	 * variable currentProccess becomes nextProcess.
	 * 
	 * @param pNextProcess
