...
public class Disk implements VoidFunctionPtr {
	...
	/**
	 * Initialize a simulated disk. Open the file (creating it if it doesn't exist),
	 * and check the magic number to make sure it's OK to treat it as JNachos disk
	 * storage.
	 * 
	 * @param name
	 *            the text name of the file simulating the Nachos disk.
	 * @param callWhenDone
	 *            the interrupt handler to be called when disk read/write request
	 *            completes.
	 * @param callArg
	...
	/**
	 * Simulate a request to read a single disk sector Do the read immediately to
	 * the file Set up an interrupt handler to be called later, that will notify the
	 * caller when the simulator says the operation has completed.
	 * 
	...
	/**
	 * Simulate a request to write a single disk sector Do the write immediately to
	 * the file Set up an interrupt handler to be called later, that will notify the
	 * caller when the simulator says the operation has completed.
	 * 
	 * Note that a disk only allows an entire sector to be written, not part of a
	 * sector.
	 * 
	 * @param sectorNumber
	 ...
	 * track on the disk. Since when we finish seeking, we are likely to be in the
	 * middle of a sector that is rotating past the head, we also return how long
	 * until the head is at the next sector boundary.
	 * 
	 * Disk seeks at one track per SeekTime ticks (cf. stats.h) and rotates at one
	 * sector per RotationTime ticks
	 */
	...
	/**
	 * Return number of sectors of rotational delay between target sector "to" and
	 * current sector position "from"
	 */
	...
	/**
	 * Return how long will it take to read/write a disk sector, from the current
	 * position of the disk head.
	 * 
	 * Latency = seek time + rotational latency + transfer time Disk seeks at one
	 * track per SeekTime ticks (cf. stats.h) and rotates at one sector per
	 * RotationTime ticks
	 * 
	 * To find the rotational latency, we first must figure out where the disk head
	 * will be after the seek (if any). We then figure out how long it will take to
	 * rotate completely past newSector after that point.
	 * 
	 * The disk also has a "track buffer"; the disk continuously reads the contents
	 * of the current disk track into the buffer. This allows read requests to the
	 * current track to be satisfied more quickly. The contents of the track buffer
	 * are discarded after every seek to a new track.
	 */
	public int computeLatency(int newSector, boolean writing) {
		...
		/*
		 * // turn this on if you don't want the track buffer stuff // check if track
		 * buffer applies if ((writing == false) && (seek == 0) && (((timeAfter -
		 * bufferInit) / RotationTime) > ModuloDiff(newSector, bufferInit /
		 * RotationTime))) { Debug.print('d', "Request latency = %d\n", RotationTime);
	...
	/**
	 * Keep track of the most recently requested sector. So we can know what is in
	 * the track buffer.
	 */
