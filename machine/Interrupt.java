...
public class Interrupt {
	 ...
	 * there can be no runnable thread, because the ready list is empty (IdleMode).
	 */
	...
	/**
	 * De-allocate the data structures needed by the interrupt simulation. Iterate
	 * through the list of pending interrupts and remove them from the lsit.
	 */
	...
	/**
	 * Change interrupts to be enabled or disabled, without advancing the simulated
	 * time (normally, enabling interrupts advances the time).
	 * 
	 * Used internally.
	 * 
	 * @param old
	...
	/**
	 * Turn interrupts on. Who cares what they used to be? Used in ThreadRoot, to
	 * turn interrupts on when first starting up a thread.
	 */
	...
	/**
	 * Advance simulated time and check if there are any pending interrupts to be
	 * called.
	 * 
	 * Two things can cause OneTick to be called: interrupts are re-enabled a user
	 ...
	 * 
	 * NOTE: the Nachos kernel should not call this routine directly. Instead, it is
	 * only called by the hardware device simulators.
	 * 
	 * @param handler
	 ...
	 * 
	 * @param advanceClock
	 *            if true, there is nothing in the ready queue, so we should simply
	 *            advance the clock to when the next pending interrupt would occur
	 *            (if any). If the pending interrupt is just the time-slice daemon,
	 *            however, then we're done!
	 */
	...
	/**
	 * Print information about an interrupt that is scheduled to occur. When, where,
	 * why, etc.
	 */
	...
	/**
	 * Print the complete interrupt state - the mStatus, and all interrupts that are
	...
	/**
	 * Inserts the pending interrupt into the list of pending interrupts in a sorted
	 * manner.
	 * 
	 * @param pInterrupt
