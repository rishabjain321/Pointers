...
public class MipsSim {
	 ...
	 * 
	 * OP_UNIMP - means that this instruction is legal, but hasn't been implemented
	 * in the simulator yet. OP_RES - means that this is a reserved opcode (it isn't
	...
	/*
	 * The table below is used to convert the "funct" field of SPECIAL instructions
	 ...
	 * handler, and when it returns, we return to Run(), which will re-invoke us in
	 * a loop. This allows us to re-start the instruction execution from the
	 ...
	 * concurrently -- one for each thread executing user code. We get re-entrancy
	 * by never caching any data -- we always re-start the simulation from scratch
	 * each time we are called (or after trapping back to the Nachos kernel on an
	 * exception or interrupt), and we always store all data back to the Machine
	 * Machine.mRegisters and memory before leaving. This allows the Nachos kernel
	 * to control our behavior by controlling the contents of memory, the
	 * translation table, and the register set.
	 */
	...
	/**
	 * Simulate R2000 multiplication. The words at *hiPtr and *loPtr are overwritten
	 * with the double-length result of the multiplication.
	 */
