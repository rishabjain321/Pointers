...
public class Timer implements VoidFunctionPtr {
	...
	/**
	 * Initialize a hardware timer device. Save the place to call on each interrupt,
	 * and then arrange for the timer to start generating interrupts.
	 * 
	 * @param pTimerHandler
	 *            is the interrupt handler for the timer device. It is called with
	 *            interrupts disabled every time the the timer expires.
	 * @param pCallArg
	 *            is the parameter to be passed to the interrupt handler.
	 * @param pDoRandom
	 *            if true, arrange for the interrupts to occur at random, instead of
	 *            fixed, intervals.
	 */
